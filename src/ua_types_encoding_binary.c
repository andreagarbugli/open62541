...
typedef status (*decodeBinarySignature)(void *UA_RESTRICT dst, const UA_DataType *type,
                                        Ctx *UA_RESTRICT ctx);
typedef size_t (*calcSizeBinarySignature)(const void *UA_RESTRICT p,
...
#define DECODE_BINARY(TYPE)                                                              \
    static status TYPE##_decodeBinary(UA_##TYPE *UA_RESTRICT dst,                        \
                                      const UA_DataType *type, Ctx *UA_RESTRICT ctx)
#define CALCSIZE_BINARY(TYPE)                                                            \
    static size_t TYPE##_calcSizeBinary(const UA_##TYPE *UA_RESTRICT src,                \
                                        const UA_DataType *_)
...
Array_encodeBinaryOverlayable(uintptr_t ptr, size_t length, size_t elementMemSize,
    ...
    while(ctx->end < ctx->pos + (elementMemSize * (length - finished))) {
        size_t possible =
            ((uintptr_t)ctx->end - (uintptr_t)ctx->pos) / (sizeof(u8) * elementMemSize);
...
DECODE_BINARY(NodeId) {
    ...
    encodingByte &=
        (u8) ~(UA_EXPANDEDNODEID_SERVERINDEX_FLAG | UA_EXPANDEDNODEID_NAMESPACEURI_FLAG);
...
DECODE_BINARY(Variant) {
    ...
    if(isArray && (encodingByte & (u8)UA_VARIANT_ENCODINGMASKTYPE_DIMENSIONS) > 0)
        ret |=
            Array_decodeBinary((void **)&dst->arrayDimensions, &dst->arrayDimensionsSize,
                               &UA_TYPES[UA_TYPES_INT32], ctx);
...
ENCODE_BINARY(DiagnosticInfo) {
    ...
    if(src->hasInnerDiagnosticInfo)
        ret = ENCODE_WITHEXCHANGE(&src->innerDiagnosticInfo, UA_TYPES_DIAGNOSTICINFO);
...
const encodeBinarySignature encodeBinaryJumpTable[UA_DATATYPEKINDS] = {
    ...
    (encodeBinarySignature)
        encodeBinaryNotImplemented,            /* Structure with Optional Fields */
    (encodeBinarySignature)encodeBinaryStruct, /* Union */
    (encodeBinarySignature)encodeBinaryStruct  /* BitfieldCluster */
};

status
UA_encodeBinary(const void *src, const UA_DataType *type, u8 **bufPos, const u8 **bufEnd,
...
const decodeBinarySignature decodeBinaryJumpTable[UA_DATATYPEKINDS] = {
    ...
    (decodeBinarySignature)
        decodeBinaryNotImplemented, /* Structure with optional fields */
...
static size_t
calcSizeBinary1(const void *_, const UA_DataType *__) {
    (void)_, (void)__;
    return 1;
}
static size_t
calcSizeBinary2(const void *_, const UA_DataType *__) {
    (void)_, (void)__;
    return 2;
}
static size_t
calcSizeBinary4(const void *_, const UA_DataType *__) {
    (void)_, (void)__;
    return 4;
}
static size_t
calcSizeBinary8(const void *_, const UA_DataType *__) {
    (void)_, (void)__;
    return 8;
}
...
CALCSIZE_BINARY(QualifiedName) { return 2 + String_calcSizeBinary(&src->name, NULL); }
...
CALCSIZE_BINARY(ExtensionObject) {
    ...
    s += NodeId_calcSizeBinary(&src->content.decoded.type->typeId,
                               NULL); /* Type encoding length */
    s += 4;                           /* Encoding length field */
    const UA_DataType *type = src->content.decoded.type;
    s += calcSizeBinaryJumpTable[type->typeKind](src->content.decoded.data,
...
calcSizeBinaryStructure(const void *p, const UA_DataType *type) {
    ...
    for(size_t i = 0; i < membersSize; ++i) {
        ...
        if(member->isArray) {
            ...
            ptr += sizeof(size_t);
            s +=
                Array_calcSizeBinary(*(void *UA_RESTRICT const *)ptr, length, membertype);
            ptr += sizeof(void *);
            continue;
        }
...
const calcSizeBinarySignature calcSizeBinaryJumpTable[UA_DATATYPEKINDS] = {
    ...
    (calcSizeBinarySignature)
        calcSizeBinaryNotImplemented, /* Structure with Optional Fields */
