...
size_t
UA_readNumberWithBase(const UA_Byte *buf, size_t buflen, UA_UInt32 *number,
                      UA_Byte base) {
    UA_assert(buf);
    UA_assert(number);
    u32 n = 0;
...
UA_StatusCode
UA_parseEndpointUrl(const UA_String *endpointUrl, UA_String *outHostname, u16 *outPort,
                    UA_String *outPath) {
...
UA_parseEndpointUrlEthernet(const UA_String *endpointUrl, UA_String *target,
    ...
    curr++; /* skip ':' */
    size_t progress =
        UA_readNumber(&endpointUrl->data[curr], endpointUrl->length - curr, &value);
...
UA_NodeId_toString(const UA_NodeId *nodeId, UA_String *nodeIdStr) {
    ...
    switch(nodeId->identifierType) {
        case UA_NODEIDTYPE_NUMERIC:
            /* ns (2 byte, 65535) = 5 chars, numeric (4 byte, 4294967295) = 10 chars,
             * delim = 1 , nullbyte = 1-> 17 chars */
        ...
        case UA_NODEIDTYPE_STRING:
            ...
            snprintfLen =
                UA_snprintf((char *)nodeIdStr->data, nodeIdStr->length, "%ss=%.*s",
                            nsLen > 0 ? nsStr : "", (int)nodeId->identifier.string.length,
                            nodeId->identifier.string.data);
            break;
        case UA_NODEIDTYPE_GUID:
            /* ns (16bit) = 5 chars + strlen(A123456C-0ABC-1A2B-815F-687212AAEE1B)=36 +
             * nullbyte */
            ...
            snprintfLen = UA_snprintf(
                (char *)nodeIdStr->data, nodeIdStr->length, "%sg=" UA_PRINTF_GUID_FORMAT,
                nsLen > 0 ? nsStr : "", UA_PRINTF_GUID_DATA(nodeId->identifier.guid));
            break;
        case UA_NODEIDTYPE_BYTESTRING:
            ...
            snprintfLen =
                UA_snprintf((char *)nodeIdStr->data, nodeIdStr->length, "%sb=%.*s",
                            nsLen > 0 ? nsStr : "", (int)byteStr.length, byteStr.data);
            UA_String_deleteMembers(&byteStr);
            break;
    }
    UA_free(nsStr);
