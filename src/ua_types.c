...
const UA_ExpandedNodeId UA_EXPANDEDNODEID_NULL = {
    {0, UA_NODEIDTYPE_NUMERIC, {0}}, {0, NULL}, 0};
...
UA_findDataType(const UA_NodeId *typeId) {
    ...
    /* TODO When other namespace look in custom types, too, requires access to custom
     * types array here! */
...
UA_String_equal(const UA_String *s1, const UA_String *s2) {
    ...
    if(s1->length == 0)
        return true;
    i32 is = memcmp((char const *)s1->data, (char const *)s2->data, s1->length);
...
UA_QualifiedName_equal(const UA_QualifiedName *qn1, const UA_QualifiedName *qn2) {
    ...
    if(qn1->name.length != qn2->name.length)
        return false;
    return (memcmp((char const *)qn1->name.data, (char const *)qn2->name.data,
                   qn1->name.length) == 0);
}
...
UA_DateTime_toStruct(UA_DateTime t) {
    ...
    long long secSinceUnixEpoch =
        (long long)((t - UA_DATETIME_UNIX_EPOCH) / UA_DATETIME_SEC);
    struct mytm ts;
...
UA_NodeId_hash(const UA_NodeId *n) {
    switch(n->identifierType) {
        case UA_NODEIDTYPE_NUMERIC:
        default:
            // shift knuth multiplication to use highest 32 bits and after addition make
            // sure we don't have an integer overflow
            return (u32)((n->namespaceIndex +
                          ((n->identifier.numeric * (u64)2654435761) >> (32))) &
                         UINT32_C(4294967295)); /*  Knuth's multiplicative hashing */
        case UA_NODEIDTYPE_STRING:
        case UA_NODEIDTYPE_BYTESTRING:
            return fnv32(n->namespaceIndex, n->identifier.string.data,
                         n->identifier.string.length);
        case UA_NODEIDTYPE_GUID:
            return fnv32(n->namespaceIndex, (const u8 *)&n->identifier.guid,
                         sizeof(UA_Guid));
    }
}
...
static UA_StatusCode
computeStrides(const UA_Variant *v, const UA_NumericRange range, size_t *total,
...
isStringLike(const UA_DataType *type) {
    if(type == &UA_TYPES[UA_TYPES_STRING] || type == &UA_TYPES[UA_TYPES_BYTESTRING] ||
...
static UA_StatusCode
copySubString(const UA_String *src, UA_String *dst, const UA_NumericRangeDimension *dim) {
    if(dim->min > dim->max)
        return UA_STATUSCODE_BADINDEXRANGEINVALID;
...
DiagnosticInfo_copy(UA_DiagnosticInfo const *src, UA_DiagnosticInfo *dst,
    ...
    if(src->hasInnerDiagnosticInfo && src->innerDiagnosticInfo) {
        dst->innerDiagnosticInfo =
            (UA_DiagnosticInfo *)UA_malloc(sizeof(UA_DiagnosticInfo));
...
static void
nopClear(void *p, const UA_DataType *type) {}

const UA_clearSignature clearJumpTable[UA_DATATYPEKINDS] = {
...
UA_NumericRange_parseFromString(UA_NumericRange *range, const UA_String *str) {
    ...
    while(true) {
        ...
        /* read the dimension */
        size_t progress =
            readDimension(&str->data[offset], str->length - offset, &dimensions[idx]);
        if(progress == 0) {
