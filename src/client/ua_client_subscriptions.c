...
UA_Client_MonitoredItems_createDataChanges(
    ...
    __UA_Client_MonitoredItems_create(client, &request, contexts,
                                      (void **)(uintptr_t)callbacks, deleteCallbacks,
                                      &response);
    return response;
}
...
UA_Client_MonitoredItems_createDataChange(
    UA_Client *client, UA_UInt32 subscriptionId, UA_TimestampsToReturn timestampsToReturn,
    ...
    UA_CreateMonitoredItemsResponse response = UA_Client_MonitoredItems_createDataChanges(
        client, request, &context, &callback, &deleteCallback);
    UA_MonitoredItemCreateResult result;
    UA_MonitoredItemCreateResult_init(&result);
    if(response.responseHeader.serviceResult != UA_STATUSCODE_GOOD)
        result.statusCode = response.responseHeader.serviceResult;

    if(result.statusCode == UA_STATUSCODE_GOOD && response.resultsSize != 1)
...
UA_Client_MonitoredItems_createEvents(
    ...
    UA_CreateMonitoredItemsResponse response;
    __UA_Client_MonitoredItems_create(client, &request, contexts,
                                      (void **)(uintptr_t)callback, deleteCallback,
                                      &response);
    return response;
}
...
UA_Client_preparePublishRequest(UA_Client *client, UA_PublishRequest *request) {
    ...
    request->subscriptionAcknowledgements =
        (UA_SubscriptionAcknowledgement *)UA_Array_new(
            request->subscriptionAcknowledgementsSize,
    ...
    LIST_FOREACH_SAFE(ack, &client->pendingNotificationsAcks, listEntry, ack_tmp) {
        request->subscriptionAcknowledgements[i].sequenceNumber =
            ack->subAck.sequenceNumber;
        request->subscriptionAcknowledgements[i].subscriptionId =
            ack->subAck.subscriptionId;
        ++i;
...
processDataChangeNotification(UA_Client *client, UA_Client_Subscription *sub,
    ...
    for(size_t j = 0; j < dataChangeNotification->monitoredItemsSize; ++j) {
        ...
        if(!mon) {
            UA_LOG_DEBUG(&client->config.logger, UA_LOGCATEGORY_CLIENT,
                         "Could not process a notification with clienthandle %u on "
                         "subscription %u",
        ...
        mon->handler.dataChangeCallback(client, sub->subscriptionId, sub->context,
                                        mon->monitoredItemId, mon->context, &min->value);
    }
}
...
processEventNotification(UA_Client *client, UA_Client_Subscription *sub,
    ...
    for(size_t j = 0; j < eventNotificationList->eventsSize; ++j) {
        ...
        mon->handler.eventCallback(
            client, sub->subscriptionId, sub->context, mon->monitoredItemId, mon->context,
            eventFieldList->eventFieldsSize, eventFieldList->eventFields);
    }
}
...
processNotificationMessage(UA_Client *client, UA_Client_Subscription *sub,
    ...
    if(msg->content.decoded.type == &UA_TYPES[UA_TYPES_STATUSCHANGENOTIFICATION]) {
        if(sub->statusChangeCallback) {
            sub->statusChangeCallback(
                client, sub->subscriptionId, sub->context,
...
UA_Client_Subscriptions_processPublishResponse(UA_Client *client,
    ...
    if(response->responseHeader.serviceResult == UA_STATUSCODE_BADSESSIONCLOSED) {
        if(client->state >= UA_CLIENTSTATE_SESSION) {
            ...
            UA_Client_Subscription *sub =
                findSubscription(client, response->subscriptionId);
            if(sub != NULL)
    ...
    if(response->responseHeader.serviceResult == UA_STATUSCODE_BADSESSIONIDINVALID) {
        UA_Client_disconnect(
            client); /* TODO: This should be handled before the process callback */
        UA_LOG_WARNING(&client->config.logger, UA_LOGCATEGORY_CLIENT,
    ...
    if(UA_Client_Subscriptions_nextSequenceNumber(sub->sequenceNumber) !=
       msg->sequenceNumber) {
        UA_LOG_WARNING(&client->config.logger, UA_LOGCATEGORY_CLIENT,
    ...
    /* According to f), a keep-alive message contains no notifications and has the
     * sequence number of the next NotificationMessage that is to be sent => More than one
     * consecutive keep-alive message or a NotificationMessage following a keep-alive
     * message will share the same sequence number. */
...
UA_Client_Subscriptions_backgroundPublishInactivityCheck(UA_Client *client) {
    ...
    UA_Client_Subscription *sub;
    LIST_FOREACH(sub, &client->subscriptions, listEntry) {
        UA_DateTime maxSilence =
            (UA_DateTime)((sub->publishingInterval * sub->maxKeepAliveCount) +
...
UA_Client_Subscriptions_backgroundPublish(UA_Client *client) {
    ...
    while(client->currentlyOutStandingPublishRequests <
          client->config.outStandingPublishRequests) {
        ...
        retval = __UA_Client_AsyncServiceEx(
            client, request, &UA_TYPES[UA_TYPES_PUBLISHREQUEST],
            processPublishResponseAsync, &UA_TYPES[UA_TYPES_PUBLISHRESPONSE],
