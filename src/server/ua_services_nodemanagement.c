...
setDeconstructedNode(UA_Server *server, UA_Session *session, UA_Node *node,
                     void *context) {
...
static UA_StatusCode
setConstructedNodeContext(UA_Server *server, UA_Session *session, UA_Node *node,
                          void *context) {
...
static UA_StatusCode
editNodeContext(UA_Server *server, UA_Session *session, UA_Node *node, void *context) {
    node->context = context;
    return UA_STATUSCODE_GOOD;
}
...
checkParentReference(UA_Server *server, UA_Session *session, UA_NodeClass nodeClass,
    ...
    /* Also, there are some variables which do not have parents, e.g. EnumStrings,
     * EnumValues */
    ...
    if(!parent) {
        UA_LOG_NODEID_WRAP(parentNodeId,
    ...
    const UA_ReferenceTypeNode *referenceType =
        (const UA_ReferenceTypeNode *)UA_Nodestore_get(server, referenceTypeId);
    if(!referenceType) {
        UA_LOG_NODEID_WRAP(
            referenceTypeId,
            UA_LOG_INFO_SESSION(&server->config.logger, session,
    ...
    if(nodeClass == UA_NODECLASS_DATATYPE || nodeClass == UA_NODECLASS_VARIABLETYPE ||
       nodeClass == UA_NODECLASS_OBJECTTYPE || nodeClass == UA_NODECLASS_REFERENCETYPE) {
    ...
    if(!isNodeInTree(&server->config.nodestore, referenceTypeId, &hierarchicalReferences,
                     &subtypeId, 1)) {
        UA_LOG_INFO_SESSION(&server->config.logger, session,
...
useVariableTypeAttributes(UA_Server *server, UA_Session *session,
    ...
    if(orig.value.type) {
        ...
        retval =
            readValueAttribute(server, session, (const UA_VariableNode *)vt, &v.value);
    ...
    if(modified) {
        const UA_VariableNode *updated =
            (const UA_VariableNode *)UA_Nodestore_get(server, &node->nodeId);
...
findChildByBrowsename(UA_Server *server, UA_Session *session,
                      const UA_NodeId *searchInstance, const UA_QualifiedName *browseName,
...
static UA_StatusCode
copyAllChildren(UA_Server *server, UA_Session *session, const UA_NodeId *source,
                const UA_NodeId *destination);
...
copyChild(UA_Server *server, UA_Session *session, const UA_NodeId *destinationNodeId,
    ...
    if(!UA_NodeId_isNull(&existingChild)) {
        if(rd->nodeClass == UA_NODECLASS_VARIABLE || rd->nodeClass == UA_NODECLASS_OBJECT)
    ...
    if(rd->nodeClass == UA_NODECLASS_VARIABLE || rd->nodeClass == UA_NODECLASS_OBJECT) {
        ...
        UA_NodeId modellingRuleReferenceId =
...
static UA_StatusCode
addTypeChildren(UA_Server *server, UA_Session *session, const UA_Node *node,
                const UA_Node *type) {
...
AddNode_addRefs(UA_Server *server, UA_Session *session, const UA_NodeId *nodeId,
    ...
    if(!UA_NodeId_isNull(typeDefinitionId)) {
        ...
        if(!type) {
            UA_LOG_NODEID_WRAP(typeDefinitionId,
                               UA_LOG_INFO_SESSION(&server->config.logger, session,
                                                   "AddNodes: Node type %.*s not found",
                                                   (int)nodeIdStr.length,
                                                   nodeIdStr.data));
        ...
        if(node->nodeClass == UA_NODECLASS_VARIABLE) {
            if(((const UA_VariableTypeNode *)type)->isAbstract) {
                ...
                getTypesHierarchy(&server->config.nodestore, parentReferences,
                                  UA_PARENT_REFERENCES_COUNT, &parentTypeHierachy,
                                  &parentTypeHierachySize, true);
                /* Abstract variable is allowed if parent is a children of a base data
                 * variable */
                const UA_NodeId variableTypes =
                    UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE);
                /* A variable may be of an object type which again is below BaseObjectType
                 */
                const UA_NodeId objectTypes =
                ...
                if(!isNodeInTree(&server->config.nodestore, parentNodeId, &variableTypes,
                   ...
                   !isNodeInTree(&server->config.nodestore, parentNodeId, &objectTypes,
                                 parentTypeHierachy, parentTypeHierachySize)) {
                    UA_Array_delete(parentTypeHierachy, parentTypeHierachySize,
                                    &UA_TYPES[UA_TYPES_NODEID]);
                    UA_LOG_NODEID_WRAP(
                        nodeId,
                        UA_LOG_INFO_SESSION(&server->config.logger, session,
    ...
    if(!UA_NodeId_isNull(parentNodeId)) {
        ...
        retval =
            addRef(server, session, &node->nodeId, referenceTypeId, parentNodeId, false);
        if(retval != UA_STATUSCODE_GOOD) {
            UA_LOG_NODEID_WRAP(
                nodeId,
                UA_LOG_INFO_SESSION(&server->config.logger, session,
    ...
    if(node->nodeClass == UA_NODECLASS_VARIABLE ||
       node->nodeClass == UA_NODECLASS_OBJECT) {
        ...
        retval = addRef(server, session, &node->nodeId, &hasTypeDefinition, &type->nodeId,
                        true);
        if(retval != UA_STATUSCODE_GOOD) {
            UA_LOG_NODEID_WRAP(nodeId, UA_LOG_INFO_SESSION(
                                           &server->config.logger, session,
...
Operation_addNode_begin(UA_Server *server, UA_Session *session, void *nodeContext,
    ...
    retval = AddNode_addRefs(server, session, outNewNodeId, parentNodeId, referenceTypeId,
...
recursiveTypeCheckAddChildren(UA_Server *server, UA_Session *session,
    ...
    if(node->nodeClass == UA_NODECLASS_VARIABLE ||
       node->nodeClass == UA_NODECLASS_VARIABLETYPE) {
        retval =
            useVariableTypeAttributes(server, session, (const UA_VariableNode **)nodeptr,
...
static UA_StatusCode
recursiveCallConstructors(UA_Server *server, UA_Session *session, const UA_Node *node,
                          const UA_Node *type) {
    if(node->constructed)
        return UA_STATUSCODE_GOOD;
    ...
    if(server->config.nodeLifecycle.constructor)
        retval = server->config.nodeLifecycle.constructor(
            server, &session->sessionId, session->sessionHandle, &node->nodeId, &context);

    /* Call the type constructor */
    if(retval == UA_STATUSCODE_GOOD && lifecycle && lifecycle->constructor)
        retval =
            lifecycle->constructor(server, &session->sessionId, session->sessionHandle,
                                   &type->nodeId, type->context, &node->nodeId, &context);
    if(retval != UA_STATUSCODE_GOOD)
        goto fail1;
...
static void
recursiveDeconstructNode(UA_Server *server, UA_Session *session, const UA_Node *node);

static void
recursiveDeleteNode(UA_Server *server, UA_Session *session, const UA_Node *node,
                    UA_Boolean removeTargetRefs);
...
constructor:
    ...
    if(retval != UA_STATUSCODE_GOOD) {
        UA_LOG_NODEID_WRAP(&node->nodeId,
                           UA_LOG_INFO_SESSION(
                               &server->config.logger, session,
...
Operation_addNode(UA_Server *server, UA_Session *session, void *nodeContext,
    ...
    result->statusCode = Operation_addNode_begin(
        server, session, nodeContext, item, &item->parentNodeId.nodeId,
...
Service_AddNodes(UA_Server *server, UA_Session *session,
    ...
    response->responseHeader.serviceResult = UA_Server_processServiceOperations(
        server, session, (UA_ServiceOperation)Operation_addNode, NULL,
...
static void
removeIncomingReferences(UA_Server *server, UA_Session *session, const UA_Node *node) {
       ...
       node->nodeClass == UA_NODECLASS_VARIABLE) {
        ...
        if(type) {
            ...
            if(lifecycle->destructor)
                lifecycle->destructor(server, &session->sessionId, session->sessionHandle,
                                      &type->nodeId, type->context, &node->nodeId,
                                      &context);
            UA_Nodestore_release(server, type);
        }
    }
...
Service_DeleteNodes(UA_Server *server, UA_Session *session,
    ...
    response->responseHeader.serviceResult = UA_Server_processServiceOperations(
        server, session, (UA_ServiceOperation)deleteNodeOperation, NULL,
...
Service_AddReferences(UA_Server *server, UA_Session *session,
    ...
    response->responseHeader.serviceResult = UA_Server_processServiceOperations(
        server, session, (UA_ServiceOperation)Operation_addReference, NULL,
...
Operation_deleteReference(UA_Server *server, UA_Session *session, void *context,
    ...
    *retval = UA_Server_editNode(
        server, session, &item->sourceNodeId, (UA_EditNodeCallback)deleteOneWayReference,
...
Service_DeleteReferences(UA_Server *server, UA_Session *session,
    ...
    response->responseHeader.serviceResult = UA_Server_processServiceOperations(
        server, session, (UA_ServiceOperation)Operation_deleteReference, NULL,
...
static UA_StatusCode
setValueCallback(UA_Server *server, UA_Session *session, UA_VariableNode *node,
...
UA_Server_addDataSourceVariableNode(
    UA_Server *server, const UA_NodeId requestedNewNodeId, const UA_NodeId parentNodeId,
    const UA_NodeId referenceTypeId, const UA_QualifiedName browseName,
    ...
    /* Create the node and add it to the nodestore */
    UA_StatusCode retval =
        AddNode_raw(server, &server->adminSession, nodeContext, &item, outNewNodeId);
    if(retval != UA_STATUSCODE_GOOD)
        goto cleanup;
...
static UA_StatusCode
setDataSource(UA_Server *server, UA_Session *session, UA_VariableNode *node,
...
UA_Server_setVariableNode_dataSource(UA_Server *server, const UA_NodeId nodeId,
    ...
    return UA_Server_editNode(
        server, &server->adminSession, &nodeId, (UA_EditNodeCallback)setDataSource,
...
static UA_StatusCode
UA_Server_addMethodNodeEx_finish(
    UA_Server *server, const UA_NodeId nodeId, UA_MethodCallback method,
    const size_t inputArgumentsSize, const UA_Argument *inputArguments,
    const UA_NodeId inputArgumentsRequestedNewNodeId,
    UA_NodeId *inputArgumentsOutNewNodeId, const size_t outputArgumentsSize,
    const UA_Argument *outputArguments, const UA_NodeId outputArgumentsRequestedNewNodeId,
    ...
    if(inputArgumentsSize > 0 && UA_NodeId_isNull(&inputArgsId)) {
        ...
        retval = UA_Server_addVariableNode(server, inputArgumentsRequestedNewNodeId,
    ...
    if(outputArgumentsSize > 0 && UA_NodeId_isNull(&outputArgsId)) {
        ...
        retval = UA_Server_addVariableNode(server, outputArgumentsRequestedNewNodeId,
                                           nodeId, hasproperty, UA_QUALIFIEDNAME(0, name),
...
UA_Server_addMethodNode_finish(UA_Server *server, const UA_NodeId nodeId,
                               UA_MethodCallback method, size_t inputArgumentsSize,
                               const UA_Argument *inputArguments,
                               size_t outputArgumentsSize,
                               const UA_Argument *outputArguments) {
    return UA_Server_addMethodNodeEx_finish(
        server, nodeId, method, inputArgumentsSize, inputArguments, UA_NODEID_NULL, NULL,
...
UA_Server_addMethodNodeEx(
    UA_Server *server, const UA_NodeId requestedNewNodeId, const UA_NodeId parentNodeId,
    const UA_NodeId referenceTypeId, const UA_QualifiedName browseName,
    const UA_MethodAttributes attr, UA_MethodCallback method, size_t inputArgumentsSize,
    const UA_Argument *inputArguments, const UA_NodeId inputArgumentsRequestedNewNodeId,
    UA_NodeId *inputArgumentsOutNewNodeId, size_t outputArgumentsSize,
    const UA_Argument *outputArguments, const UA_NodeId outputArgumentsRequestedNewNodeId,
    UA_NodeId *outputArgumentsOutNewNodeId, void *nodeContext, UA_NodeId *outNewNodeId) {
    UA_AddNodesItem item;
    UA_AddNodesItem_init(&item);
    ...
    retval = UA_Server_addMethodNodeEx_finish(
        server, *outNewNodeId, method, inputArgumentsSize, inputArguments,
        inputArgumentsRequestedNewNodeId, inputArgumentsOutNewNodeId, outputArgumentsSize,
        outputArguments, outputArgumentsRequestedNewNodeId, outputArgumentsOutNewNodeId);
...
static UA_StatusCode
editMethodCallback(UA_Server *server, UA_Session *session, UA_Node *node, void *handle) {
...
static UA_StatusCode
setNodeTypeLifecycle(UA_Server *server, UA_Session *session, UA_Node *node,
...
UA_Server_setNodeTypeLifecycle(UA_Server *server, UA_NodeId nodeId,
                               UA_NodeTypeLifecycle lifecycle) {
    return UA_Server_editNode(server, &server->adminSession, &nodeId,
                              (UA_EditNodeCallback)setNodeTypeLifecycle, &lifecycle);
}
