...
UA_Server_createEvent(UA_Server *server, const UA_NodeId eventType,
    ...
    if(!isNodeInTree(&server->config.nodestore, &eventType, &baseEventTypeId,
                     &hasSubtypeId, 1)) {
        UA_LOG_ERROR(&server->config.logger, UA_LOGCATEGORY_USERLAND,
    ...
    UA_StatusCode retval =
        UA_Server_addObjectNode(server, UA_NODEID_NULL, /* Set a random unused NodeId */
    ...
    UA_BrowsePathResult bpr =
        UA_Server_browseSimplifiedBrowsePath(server, newNodeId, 1, &name);
    ...
    UA_Variant_setScalar(&value, (void *)(uintptr_t)&eventType,
...
isValidEvent(UA_Server *server, const UA_NodeId *validEventParent,
    ...
    if(UA_NodeId_equal(validEventParent, &conditionTypeId) ||
       isNodeInTree(&server->config.nodestore, tEventType, &conditionTypeId,
                    &hasSubtypeId, 1)) {
        UA_LOG_ERROR(&server->config.logger, UA_LOGCATEGORY_USERLAND,
...
resolveSimpleAttributeOperand(UA_Server *server, UA_Session *session,
    ...
    UA_BrowsePathResult bpr = UA_Server_browseSimplifiedBrowsePath(
        server, *origin, sao->browsePathSize, sao->browsePath);
    ...
    UA_DataValue v =
        UA_Server_readWithSession(server, session, &rvi, UA_TIMESTAMPSTORETURN_NEITHER);
    if(v.status == UA_STATUSCODE_GOOD && v.hasValue)
        *value = v.value;
...
UA_Server_filterEvent(UA_Server *server, UA_Session *session, const UA_NodeId *eventNode,
                      UA_EventFilter *filter, UA_EventNotification *notification) {
    ...
    notification->fields.eventFields = (UA_Variant *)UA_Array_new(
        notification->fields.eventFieldsSize, &UA_TYPES[UA_TYPES_VARIANT]);
    ...
    */
    ...
    /* ================ apply the filter ===================== */
    /* check if the browsePath is BaseEventType, in which case nothing more needs to be
     * checked */
    UA_NodeId baseEventTypeId = UA_NODEID_NUMERIC(0, UA_NS0ID_BASEEVENTTYPE);
    /* iterate over the selectClauses */
    for(size_t i = 0; i < filter->selectClausesSize; i++) {
        if(!UA_NodeId_equal(&filter->selectClauses[i].typeDefinitionId,
                            &baseEventTypeId) &&
    ...
    /* return whereClausesApply(server, filter->whereClause, &notification->fields,
     * &whereClauseResult); */
    return UA_STATUSCODE_GOOD;
}

static UA_StatusCode
eventSetStandardFields(UA_Server *server, const UA_NodeId *event, const UA_NodeId *origin,
                       UA_ByteString *outEventId) {
    /* Set the SourceNode */
    UA_StatusCode retval;
    UA_QualifiedName name = UA_QUALIFIEDNAME(0, "SourceNode");
    UA_BrowsePathResult bpr =
        UA_Server_browseSimplifiedBrowsePath(server, *event, 1, &name);
    if(bpr.statusCode != UA_STATUSCODE_GOOD || bpr.targetsSize < 1) {
        retval = bpr.statusCode;
        UA_BrowsePathResult_deleteMembers(&bpr);
        return retval;
...
getParentsNodeIteratorCallback(UA_NodeId parentId, UA_Boolean isInverse,
    ...
    Events_nodeListElement *entry =
        (Events_nodeListElement *)UA_malloc(sizeof(Events_nodeListElement));
    if(!entry)
        return UA_STATUSCODE_BADOUTOFMEMORY;
    ...
    UA_Server_forEachChildNodeCall(
        handle->server, parentId, (UA_NodeIteratorCallback)getParentsNodeIteratorCallback,
        handle);
    return UA_STATUSCODE_GOOD;
}
...
UA_Event_addEventToMonitoredItem(UA_Server *server, const UA_NodeId *event,
    ...
    UA_StatusCode retval = UA_Server_filterEvent(
        server, session, event, &mon->filter.eventFilter, &notification->data.event);
    if(retval != UA_STATUSCODE_GOOD) {
...
UA_Server_triggerEvent(UA_Server *server, const UA_NodeId eventNodeId,
    ...
    UA_Boolean isInObjectsFolder =
        isNodeInTree(&server->config.nodestore, &origin, &objectsFolderId,
    ...
    UA_StatusCode retval =
        eventSetStandardFields(server, &eventNodeId, &origin, outEventId);
    if(retval != UA_STATUSCODE_GOOD) {
        UA_LOG_WARNING(
    ...
    LIST_FOREACH_SAFE(parentIter, &parentHandle.nodes, listEntry, tmp_parentIter) {
        const UA_ObjectNode *node =
            (const UA_ObjectNode *)UA_Nodestore_get(server, &parentIter->nodeId);
        if(node->nodeClass == UA_NODECLASS_OBJECT) {
            for(UA_MonitoredItem *monIter = node->monitoredItemQueue; monIter != NULL;
                monIter = monIter->next) {
                retval = UA_Event_addEventToMonitoredItem(server, &eventNodeId, monIter);
                if(retval != UA_STATUSCODE_GOOD) {
                    UA_LOG_WARNING(&server->config.logger, UA_LOGCATEGORY_SERVER,
                                   "Events: Could not add the event to a listening node "
                                   "with StatusCode %s",
                                   UA_StatusCode_name(retval));
                }
            }
