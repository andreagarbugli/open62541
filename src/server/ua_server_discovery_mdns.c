...
mdns_record_add_or_get(UA_DiscoveryManager *dm, const char *record,
    ...
    while(hash_entry) {
        ...
        if(strncmp((char *)hash_entry->entry->serverOnNetwork.serverName.data, serverName,
                   maxLen) == 0)
    ...
    struct serverOnNetwork_list_entry *listEntry =
        (serverOnNetwork_list_entry *)UA_malloc(
            sizeof(struct serverOnNetwork_list_entry));
    ...
    struct serverOnNetwork_hash_entry *newHashEntry =
        (struct serverOnNetwork_hash_entry *)UA_malloc(
            sizeof(struct serverOnNetwork_hash_entry));
...
getInterfaces(UA_Server *server) {
    ...
    for(size_t attempts = 0; attempts != 3; ++attempts) {
        /* todo: malloc may fail: return a statuscode */
        adapter_addresses =
            (IP_ADAPTER_ADDRESSES *)UA_malloc(adapter_addresses_buffer_size);
...
mdns_is_self_announce(UA_Server *server, struct serverOnNetwork_list_entry *entry) {
    ...
    UA_StatusCode retval = UA_parseEndpointUrl(&entry->serverOnNetwork.discoveryUrl,
    ...
    for(size_t i = 0; i < server->config.networkLayersSize; i++) {
        ...
        retval =
            UA_parseEndpointUrl(&nl->discoveryUrl, &hostnameSelf, &portSelf, &pathSelf);
        if(retval != UA_STATUSCODE_GOOD) {
            /* skip invalid url */
            continue;
        }
        ...
        for(; adapter != NULL; adapter = adapter->Next) {
            ...
            for(; NULL != address; address = address->Next) {
                ...
                if(AF_INET == family) {
                    SOCKADDR_IN *ipv4 =
                        (SOCKADDR_IN *)(address->Address.lpSockaddr); /* IPv4 */
...
mdns_record_remove(UA_Server *server, const char *record,
    ...
    if(dm->serverOnNetworkCallback && !mdns_is_self_announce(server, entry))
        dm->serverOnNetworkCallback(&entry->serverOnNetwork, false, entry->txtSet,
...
mdns_record_received(const struct resource *r, void *data) {
    ...
    /* Get entry */
    struct serverOnNetwork_list_entry *entry = mdns_record_add_or_get(
        &server->discoveryManager, r->name, r->name, servernameLen, r->ttl > 0);
    if(!entry)
        return;
    ...
    if(entry->srvSet && server->discoveryManager.serverOnNetworkCallback &&
       !mdns_is_self_announce(server, entry))
        server->discoveryManager.serverOnNetworkCallback(
            &entry->serverOnNetwork, true, entry->txtSet,
...
mdns_create_txt(UA_Server *server, const char *fullServiceDomain, const char *path,
    ...
    mdns_record_t *r = mdnsd_unique(server->discoveryManager.mdnsDaemon,
                                    fullServiceDomain, QTYPE_TXT, 600, conflict, server);
    xht_t *h = xht_new(11);
    char *allocPath = NULL;
