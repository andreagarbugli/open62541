...
sendServiceFault(UA_SecureChannel *channel, const UA_ByteString *msg, size_t offset,
...
getServicePointers(UA_UInt32 requestTypeId, const UA_DataType **requestType,
                   ...
                   UA_InSituService *serviceInsitu, UA_Boolean *requiresSession,
                   UA_ServiceType *serviceType) {
...
processHEL(UA_Server *server, UA_Connection *connection, const UA_ByteString *msg,
    ...
    memcpy(&ackMessage, localConfig,
           sizeof(UA_TcpAcknowledgeMessage)); /* Same struct layout.. */
...
static UA_StatusCode
processOPN(UA_Server *server, UA_SecureChannel *channel, const UA_UInt32 requestId,
    ...
    retval =
        UA_OpenSecureChannelRequest_decodeBinary(msg, &offset, &openSecureChannelRequest);

    /* Error occurred */
    if(retval != UA_STATUSCODE_GOOD ||
       requestType.identifier.numeric !=
    ...
    Service_OpenSecureChannel(server, channel, &openSecureChannelRequest,
                              &openScResponse);
    UA_OpenSecureChannelRequest_deleteMembers(&openSecureChannelRequest);
    ...
    /* Send the response */
    retval = UA_SecureChannel_sendAsymmetricOPNMessage(
        channel, requestId, &openScResponse,
...
static UA_StatusCode
processMSG(UA_Server *server, UA_SecureChannel *channel, UA_UInt32 requestId,
           const UA_ByteString *msg) {
    ...
    getServicePointers(requestTypeId.identifier.numeric, &requestType, &responseType,
                       &service, &serviceInsitu, &sessionRequired, &serviceType);
    if(!requestType) {
    ...
    if(retval != UA_STATUSCODE_GOOD) {
        ...
        return sendServiceFault(channel, msg, requestPos, responseType, requestId,
                                retval);
    }
    ...
    if(requestType == &UA_TYPES[UA_TYPES_CREATESESSIONREQUEST]) {
        Service_CreateSession(server, channel, (const UA_CreateSessionRequest *)request,
...
#ifdef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
    // set the authenticationToken from the create session request to help fuzzing cover
    // more lines
    ...
    session = (UA_Session *)UA_SecureChannel_getSession(
        channel, &requestHeader->authenticationToken);
    if(!session && !UA_NodeId_isNull(&requestHeader->authenticationToken))
        session = UA_SessionManager_getSessionByToken(
            &server->sessionManager, &requestHeader->authenticationToken);
    ...
    if(requestType == &UA_TYPES[UA_TYPES_PUBLISHREQUEST]) {
        Service_Publish(server, session, (const UA_PublishRequest *)request, requestId);
        UA_deleteMembers(request, requestType);
...
send_response:
    ...
    switch(serviceType) {
        ...
        case UA_SERVICETYPE_INSITU:
            retval = serviceInsitu(server, session, &mc, request,
...
createSecureChannel(void *application, UA_Connection *connection,
    ...
    for(size_t i = 0; i < server->config.endpointsSize; ++i) {
        ...
        securityPolicy = UA_SecurityPolicy_getSecurityPolicyByUri(
            server, (UA_ByteString *)&endpointCandidate->securityPolicyUri);
        if(!securityPolicy)
            return UA_STATUSCODE_BADINTERNALERROR;

        UA_StatusCode retval =
            securityPolicy->asymmetricModule.compareCertificateThumbprint(
...
processCompleteChunkWithoutChannel(UA_Server *server, UA_Connection *connection,
    ...
    switch(tcpMessageHeader.messageTypeAndChunkType & 0x00ffffffu) {
        ...
        case UA_MESSAGETYPE_OPN: {
            UA_LOG_TRACE(&server->config.logger, UA_LOGCATEGORY_NETWORK,
                         "Connection %i | Process OPN message",
                         (int)(connection->sockfd));
            ...
            // Decode the asymmetric algorithm security header since it is not encrypted
            // and needed to decide what security policy to use.
            UA_AsymmetricAlgorithmSecurityHeader asymHeader;
            UA_AsymmetricAlgorithmSecurityHeader_init(&asymHeader);
            size_t messageHeaderOffset = UA_SECURE_CONVERSATION_MESSAGE_HEADER_LENGTH;
            retval = UA_AsymmetricAlgorithmSecurityHeader_decodeBinary(
                message, &messageHeaderOffset, &asymHeader);
            if(retval != UA_STATUSCODE_GOOD)
                break;
            ...
            retval =
                UA_SecureChannel_decryptAddChunk(connection->channel, message, false);
            if(retval != UA_STATUSCODE_GOOD)
                break;
...
UA_Server_processBinaryMessage(UA_Server *server, UA_Connection *connection,
    ...
    UA_StatusCode retval =
        UA_Connection_processChunks(connection, server, processCompleteChunk, message);
    if(retval != UA_STATUSCODE_GOOD) {
        UA_LOG_INFO(&server->config.logger, UA_LOGCATEGORY_NETWORK,
    ...
    UA_SecureChannel_processCompleteMessages(channel, server,
                                             processSecureChannelMessage);
