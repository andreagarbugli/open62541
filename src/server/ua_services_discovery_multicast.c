...
multicastWorkerLoop(UA_Server *server) {
    ...
    while(*running) {
        ...
        unsigned short retVal = mdnsd_step(
            server->discoveryManager.mdnsDaemon, server->discoveryManager.mdnsSocket,
...
addMdnsRecordForNetworkLayer(UA_Server *server, const UA_String *appName,
    ...
    UA_StatusCode retval =
        UA_parseEndpointUrl(&nl->discoveryUrl, &hostname, &port, &path);
    if(retval != UA_STATUSCODE_GOOD) {
        UA_LOG_WARNING(&server->config.logger, UA_LOGCATEGORY_NETWORK,
                       "Server url is invalid: %.*s", (int)nl->discoveryUrl.length,
                       nl->discoveryUrl.data);
        return retval;
    }
    UA_Discovery_addRecord(server, appName, &hostname, port, &path, UA_DISCOVERY_TCP,
                           true, server->config.serverCapabilities,
...
stopMulticastDiscoveryServer(UA_Server *server) {
    ...
    if(UA_gethostname(hostname, 255) == 0) {
        ...
        UA_Discovery_removeRecord(server, &server->config.mdnsServerName, &hnString, 4840,
                                  true);
    } else {
        UA_LOG_ERROR(&server->config.logger, UA_LOGCATEGORY_SERVER,
...
UA_Server_updateMdnsForDiscoveryUrl(UA_Server *server, const UA_String *serverName,
    ...
    if(retval != UA_STATUSCODE_GOOD) {
        UA_LOG_WARNING(&server->config.logger, UA_LOGCATEGORY_NETWORK,
                       "Server url invalid: %.*s", (int)discoveryUrl->length,
                       discoveryUrl->data);
        return;
    }
    ...
    if(!isOnline) {
        UA_StatusCode removeRetval =
            UA_Discovery_removeRecord(server, serverName, &hostname, port, updateTxt);
    ...
    UA_StatusCode addRetval = UA_Discovery_addRecord(server, serverName, &hostname, port,
                                                     &path, UA_DISCOVERY_TCP, updateTxt,
...
createFullServiceDomain(char *outServiceDomain, size_t maxLen,
    ...
    size_t offset = 0;
    if(hostnameLen > 0) {
        UA_snprintf(outServiceDomain, maxLen + 1, "%.*s-%.*s", (int)servernameLen,
                    (char *)servername->data, (int)hostnameLen, (char *)hostname->data);
        offset = servernameLen + hostnameLen + 1;
    } else {
        UA_snprintf(outServiceDomain, maxLen + 1, "%.*s", (int)servernameLen,
...
discovery_multicastQueryAnswer(mdns_answer_t *a, void *arg) {
    ...
    UA_Boolean exists = UA_Discovery_recordExists(server, a->rdname, 0, UA_DISCOVERY_TCP);
    if(exists == true)
        return 0;
...
UA_Discovery_multicastQuery(UA_Server *server) {
    mdnsd_query(server->discoveryManager.mdnsDaemon, "_opcua-tcp._tcp.local.", QTYPE_PTR,
                discovery_multicastQueryAnswer, server);
    return UA_STATUSCODE_GOOD;
}
...
UA_Discovery_addRecord(UA_Server *server, const UA_String *servername,
    ...
    if(hostnameLen + servernameLen + 1 >
       63) {  // include dash between servername-hostname
        UA_LOG_WARNING(&server->config.logger, UA_LOGCATEGORY_SERVER,
    ...
    if(!server->discoveryManager.mdnsMainSrvAdded) {
        mdns_record_t *r = mdnsd_shared(server->discoveryManager.mdnsDaemon,
                                        "_services._dns-sd._udp.local.", QTYPE_PTR, 600);
    ...
    UA_Boolean exists =
        UA_Discovery_recordExists(server, fullServiceDomain, port, protocol);
    if(exists == true)
        return UA_STATUSCODE_GOOD;
    ...
    if(!r) {
        r = mdnsd_shared(server->discoveryManager.mdnsDaemon, "_opcua-tcp._tcp.local.",
                         QTYPE_PTR, 600);
    ...
    // [servername]-[hostname]._opcua-tcp._tcp.local. 86400 IN SRV 0 5 port [hostname].
    r = mdnsd_unique(server->discoveryManager.mdnsDaemon, fullServiceDomain, QTYPE_SRV,
    ...
    // TXT record: [servername]-[hostname]._opcua-tcp._tcp.local. TXT path=/
    // caps=NA,DA,...
...
UA_Discovery_removeRecord(UA_Server *server, const UA_String *servername,
    ...
    if(!r) {
        UA_LOG_WARNING(&server->config.logger, UA_LOGCATEGORY_SERVER,
                       ...
                       "PTR Record not found for domain: %s",
                       fullServiceDomain);
        return UA_STATUSCODE_BADNOTHINGTODO;
    }
    mdnsd_done(server->discoveryManager.mdnsDaemon, r);

    // looks for [servername]-[hostname]._opcua-tcp._tcp.local. 86400 IN SRV 0 5 port
    // hostname.local. and TXT record: [servername]-[hostname]._opcua-tcp._tcp.local. TXT
    // path=/ caps=NA,DA,... and A record: [servername]-[hostname]._opcua-tcp._tcp.local.
    // A [ip]
    mdns_record_t *r2 =
        mdnsd_get_published(server->discoveryManager.mdnsDaemon, fullServiceDomain);
    if(!r2) {
    ...
    while(r2) {
        ...
        if((removeTxt && data->type == QTYPE_TXT) ||
           (removeTxt && data->type == QTYPE_A) || data->srv.port == port) {
...
iterateMulticastDiscoveryServer(UA_Server *server, UA_DateTime *nextRepeat,
    ...
    unsigned short retval =
        mdnsd_step(server->discoveryManager.mdnsDaemon,
                   server->discoveryManager.mdnsSocket, processIn, true, &next_sleep);
    if(retval == 1) {
        UA_LOG_SOCKET_ERRNO_WRAP(
            UA_LOG_DEBUG(&server->config.logger, UA_LOGCATEGORY_SERVER,
                         "Multicast error: Can not read from socket. %s", errno_str));
    ...
    if(nextRepeat)
        *nextRepeat =
            UA_DateTime_now() + (UA_DateTime)((next_sleep.tv_sec * UA_DATETIME_SEC) +
