...
getArgumentsVariableNode(UA_Server *server, const UA_MethodNode *ofMethod,
    ...
    for(size_t i = 0; i < ofMethod->referencesSize; ++i) {
        ...
        for(size_t j = 0; j < rk->targetIdsSize; ++j) {
            const UA_Node *refTarget = server->config.nodestore.getNode(
                server->config.nodestore.context, &rk->targetIds[j].nodeId);
            if(!refTarget)
                continue;
...
static const UA_NodeId hasComponentNodeId = {
...
callWithMethodAndObject(UA_Server *server, UA_Session *session,
    ...
    result->inputArgumentResults = (UA_StatusCode *)UA_Array_new(
        request->inputArgumentsSize, &UA_TYPES[UA_TYPES_STATUSCODE]);
    ...
    result->outputArguments =
        (UA_Variant *)UA_Array_new(outputArgsSize, &UA_TYPES[UA_TYPES_VARIANT]);
    ...
    /* Call the method */
    result->statusCode = method->method(
        server, &session->sessionId, session->sessionHandle, &method->nodeId,
        method->context, &object->nodeId, object->context, request->inputArgumentsSize,
        request->inputArguments, result->outputArgumentsSize, result->outputArguments);
...
Operation_CallMethod(UA_Server *server, UA_Session *session, void *context,
                     const UA_CallMethodRequest *request, UA_CallMethodResult *result) {
    /* Get the method node */
    const UA_MethodNode *method = (const UA_MethodNode *)server->config.nodestore.getNode(
        server->config.nodestore.context, &request->methodId);
    if(!method) {
    ...
    /* Get the object node */
    const UA_ObjectNode *object = (const UA_ObjectNode *)server->config.nodestore.getNode(
        server->config.nodestore.context, &request->objectId);
    if(!object) {
        result->statusCode = UA_STATUSCODE_BADNODEIDUNKNOWN;
        server->config.nodestore.releaseNode(server->config.nodestore.context,
...
void
Service_Call(UA_Server *server, UA_Session *session, const UA_CallRequest *request,
    ...
    response->responseHeader.serviceResult = UA_Server_processServiceOperations(
        server, session, (UA_ServiceOperation)Operation_CallMethod, NULL,
