...
updateNeededForFilteredPercentValue(const UA_Variant *value, const UA_Variant *oldValue,
    ...
    if(UA_Variant_isScalar(value)) {
        return outOfPercentDeadBand(value->data, oldValue->data, 0, value->type,
                                    deadbandValue, euRange);
    }
    for(size_t i = 0; i < value->arrayLength; ++i) {
        if(outOfPercentDeadBand(value->data, oldValue->data, i, value->type,
                                deadbandValue, euRange))
            return true;
    }
    return false;
...
detectValueChangeWithFilter(UA_Server *server, UA_MonitoredItem *mon, UA_DataValue *value,
    ...
    if(UA_DataType_isNumeric(value->value.type) &&
       (mon->filter.dataChangeFilter.trigger == UA_DATACHANGETRIGGER_STATUSVALUE ||
        mon->filter.dataChangeFilter.trigger ==
        ...
        if(mon->filter.dataChangeFilter.deadbandType == UA_DEADBANDTYPE_ABSOLUTE) {
            ...
            UA_BrowsePathResult bpr = UA_Server_browseSimplifiedBrowsePath(
                server, mon->monitoredNodeId, 1, &qn);
            if(bpr.statusCode != UA_STATUSCODE_GOOD ||
               bpr.targetsSize <
                   1) {  // if branch is not entried, property has been found
                UA_BrowsePathResult_deleteMembers(&bpr);
                return UA_STATUSCODE_GOOD;
            }
            const UA_VariableNode *node = (const UA_VariableNode *)UA_Nodestore_get(
                server, &bpr.targets->targetId.nodeId);
            UA_Range *euRange = (UA_Range *)node->value.data.value.value.data;
            if(!updateNeededForFilteredPercentValue(
                   &value->value, &mon->lastValue,
                   mon->filter.dataChangeFilter.deadbandValue, euRange)) {
                if(!updateNeededForStatusCode(
                       value,
                       mon))  // when same value, but different status code is written
                    return UA_STATUSCODE_GOOD;
            }
        }
#endif
    ...
    UA_StatusCode retval = UA_encodeBinary(value, &UA_TYPES[UA_TYPES_DATAVALUE], &bufPos,
                                           &bufEnd, NULL, NULL);
    if(retval == UA_STATUSCODE_BADENCODINGERROR) {
        ...
        if(binsize > UA_VALUENCODING_MAXSTACK) {
            ...
            if(retval == UA_STATUSCODE_GOOD) {
                ...
                retval = UA_encodeBinary(value, &UA_TYPES[UA_TYPES_DATAVALUE], &bufPos,
                                         &bufEnd, NULL, NULL);
            }
        }
    }
...
static UA_StatusCode
detectValueChange(UA_Server *server, UA_MonitoredItem *mon, UA_DataValue value,
                  UA_ByteString *encoding, UA_Boolean *changed) {
    /* Apply Filter */
 ...
 * default is false. */
static UA_StatusCode
sampleCallbackWithValue(UA_Server *server, UA_Session *session, UA_Subscription *sub,
                        UA_MonitoredItem *mon, UA_DataValue *value,
                        UA_Boolean *movedValue) {
    ...
    UA_StatusCode retval =
        detectValueChange(server, mon, *value, &binValueEncoding, &changed);
    if(retval != UA_STATUSCODE_GOOD) {
    ...
    if(!sub) {
        ...
        localMon->callback.dataChangeCallback(server, mon->monitoredItemId,
                                              localMon->context, &mon->monitoredNodeId,
                                              nodeContext, mon->attributeId, value);
    }
...
UA_MonitoredItem_sampleCallback(UA_Server *server, UA_MonitoredItem *monitoredItem) {
    ...
    if(node) {
        ...
        ReadWithNode(node, server, session, monitoredItem->timestampsToReturn, &rvid,
                     &value);
    } else {
    ...
    UA_StatusCode retval =
        sampleCallbackWithValue(server, session, sub, monitoredItem, &value, &movedValue);
    if(retval != UA_STATUSCODE_GOOD) {
    ...
    if(!movedValue)
        UA_DataValue_deleteMembers(
            &value); /* Does nothing for UA_VARIANT_DATA_NODELETE */
    if(node)
