...
getUserWriteMask(UA_Server *server, const UA_Session *session, const UA_Node *node) {
    ...
    return node->writeMask & server->config.accessControl.getUserRightsMask(
...
getUserAccessLevel(UA_Server *server, const UA_Session *session,
    ...
    if(session == &server->adminSession)
        return 0xFF; /* the local admin user has all rights */
    return node->accessLevel & server->config.accessControl.getUserAccessLevel(
                                   server, &server->config.accessControl,
                                   &session->sessionId, session->sessionHandle,
                                   &node->nodeId, node->context);
}
...
getUserExecutable(UA_Server *server, const UA_Session *session,
    ...
    return node->executable & server->config.accessControl.getUserExecutable(
                                  server, &server->config.accessControl,
                                  &session->sessionId, session->sessionHandle,
                                  &node->nodeId, node->context);
}
...
static void
setScalarNoDelete(UA_Variant *v, const void *UA_RESTRICT p, const UA_DataType *type) {
...
readValueAttributeFromNode(UA_Server *server, UA_Session *session,
    ...
    if(vn->value.data.callback.onRead) {
        vn->value.data.callback.onRead(server, &session->sessionId,
                                       session->sessionHandle, &vn->nodeId, vn->context,
                                       rangeptr, &vn->value.data.value);
...
readValueAttributeFromDataSource(UA_Server *server, UA_Session *session,
    ...
    return vn->value.dataSource.read(server, &session->sessionId, session->sessionHandle,
                                     &vn->nodeId, vn->context, sourceTimeStamp, rangeptr,
                                     v);
}
...
ReadWithNode(const UA_Node *node, UA_Server *server, UA_Session *session,
             ...
             UA_DataValue *v) {
    UA_LOG_DEBUG_SESSION(&server->config.logger, session, "Read the attribute %i",
                         id->attributeId);
    ...
    switch(id->attributeId) {
        ...
        case UA_ATTRIBUTEID_BROWSENAME:
            setScalarNoDelete(&v->value, &node->browseName,
                              &UA_TYPES[UA_TYPES_QUALIFIEDNAME]);
            break;
        case UA_ATTRIBUTEID_DISPLAYNAME:
            setScalarNoDelete(&v->value, &node->displayName,
                              &UA_TYPES[UA_TYPES_LOCALIZEDTEXT]);
            break;
        case UA_ATTRIBUTEID_DESCRIPTION:
            setScalarNoDelete(&v->value, &node->description,
                              &UA_TYPES[UA_TYPES_LOCALIZEDTEXT]);
            break;
        ...
        case UA_ATTRIBUTEID_EVENTNOTIFIER:
            ...
            if(node->nodeClass == UA_NODECLASS_VIEW) {
                ...
                setScalarNoDelete(&v->value,
                                  &((const UA_ObjectNode *)node)->eventNotifier,
                                  &UA_TYPES[UA_TYPES_BYTE]);
            }
            break;
        case UA_ATTRIBUTEID_VALUE: {
            CHECK_NODECLASS(UA_NODECLASS_VARIABLE | UA_NODECLASS_VARIABLETYPE);
            /* VariableTypes don't have the AccessLevel concept. Always allow reading the
             * value. */
            if(node->nodeClass == UA_NODECLASS_VARIABLE) {
                /* The access to a value variable is granted via the AccessLevel
                 * and UserAccessLevel attributes */
                UA_Byte accessLevel =
                    getAccessLevel(server, session, (const UA_VariableNode *)node);
                if(!(accessLevel & (UA_ACCESSLEVELMASK_READ))) {
                    retval = UA_STATUSCODE_BADNOTREADABLE;
                    break;
                }
                accessLevel =
                    getUserAccessLevel(server, session, (const UA_VariableNode *)node);
                if(!(accessLevel & (UA_ACCESSLEVELMASK_READ))) {
            ...
            retval =
                readValueAttributeComplete(server, session, (const UA_VariableNode *)node,
        ...
        case UA_ATTRIBUTEID_USEREXECUTABLE: {
            ...
            UA_Boolean userExecutable =
                getUserExecutable(server, session, (const UA_MethodNode *)node);
...
Operation_Read(UA_Server *server, UA_Session *session, UA_MessageContext *mc,
    ...
    UA_StatusCode retval =
        UA_MessageContext_encode(mc, &dv, &UA_TYPES[UA_TYPES_DATAVALUE]);
...
compatibleDataType(UA_Server *server, const UA_NodeId *dataType,
    ...
    if(isNodeInTree(&server->config.nodestore, dataType, constraintDataType, &subtypeId,
                    1))
        return true;

    /* Enum allows Int32 (only) */
    if(UA_NodeId_equal(dataType, &UA_TYPES[UA_TYPES_INT32].typeId) &&
       isNodeInTree(&server->config.nodestore, constraintDataType, &enumNodeId,
                    &subtypeId, 1))
        return true;
    ...
    if(isValue) {
        ...
        if(dataType->namespaceIndex == 0 &&
           ...
           isNodeInTree(&server->config.nodestore, constraintDataType, dataType,
                        &subtypeId, 1))
            return true;
    }
...
compatibleValueRankArrayDimensions(UA_Server *server, UA_Session *session,
    ...
    /* case -3, UA_VALUERANK_SCALAR_OR_ONE_DIMENSION: the value can be a scalar or a one
     * dimensional array */
    /* case -2, UA_VALUERANK_ANY: the value can be a scalar or an array with any number of
     * dimensions */
    /* case -1, UA_VALUERANK_SCALAR: the value is a scalar */
    /* case  0, UA_VALUERANK_ONE_OR_MORE_DIMENSIONS:  the value is an array with one or
     * more dimensions */
    ...
    /* case >= 1, UA_VALUERANK_ONE_DIMENSION: the value is an array with the specified
...
compatibleValueRanks(UA_Int32 valueRank, UA_Int32 constraintValueRank) {
    ...
    switch(constraintValueRank) {
        case UA_VALUERANK_SCALAR_OR_ONE_DIMENSION: /* the value can be a scalar or a one
                                                      dimensional array */
            if(valueRank != UA_VALUERANK_SCALAR &&
               valueRank != UA_VALUERANK_ONE_DIMENSION)
                return false;
            break;
        case UA_VALUERANK_ANY: /* the value can be a scalar or an array with any number of
                                  dimensions */
            break;
        case UA_VALUERANK_SCALAR: /* the value is a scalar */
            if(valueRank != UA_VALUERANK_SCALAR)
                return false;
            break;
        case UA_VALUERANK_ONE_OR_MORE_DIMENSIONS: /* the value is an array with one or
                                                     more dimensions */
    ...
    if(node->valueSource == UA_VALUESOURCE_DATA) {
        ...
        if(retval == UA_STATUSCODE_GOOD && node->nodeClass == UA_NODECLASS_VARIABLE &&
           server->config.historyDatabase.setValue)
            server->config.historyDatabase.setValue(
                ...
                session->sessionHandle, &node->nodeId, node->historizing, &adjustedValue);
#endif
        ...
        if(node->value.dataSource.write) {
            retval = node->value.dataSource.write(
                server, &session->sessionId, session->sessionHandle, &node->nodeId,
                node->context, rangeptr, &adjustedValue);
        } else {
...
#define GET_NODETYPE                                                                     \
    type = (const UA_VariableTypeNode *)getNodeType(server, node);                       \
    if(!type) {                                                                          \
        retval = UA_STATUSCODE_BADTYPEMISMATCH;                                          \
        break;                                                                           \
    }
...
copyAttributeIntoNode(UA_Server *server, UA_Session *session, UA_Node *node,
    ...
    switch(wvalue->attributeId) {
        ...
        case UA_ATTRIBUTEID_ARRAYDIMENSIONS:
            ...
            GET_NODETYPE
            retval = writeArrayDimensionsAttribute(
                server, session, (UA_VariableNode *)node, type,
                wvalue->value.value.arrayLength, (UA_UInt32 *)wvalue->value.value.data);
            UA_Nodestore_release(server, (const UA_Node *)type);
            break;
        case UA_ATTRIBUTEID_ACCESSLEVEL:
...
static void
Operation_Write(UA_Server *server, UA_Session *session, void *context, UA_WriteValue *wv,
...
UA_Server_writeWithSession(UA_Server *server, UA_Session *session,
    ...
    return UA_Server_editNode(
        server, session, &value->nodeId, (UA_EditNodeCallback)copyAttributeIntoNode,
...
UA_Server_write(UA_Server *server, const UA_WriteValue *value) {
    return UA_Server_editNode(
        server, &server->adminSession, &value->nodeId,
...
Service_HistoryRead(UA_Server *server, UA_Session *session,
    ...
    UA_ReadRawModifiedDetails *details =
        (UA_ReadRawModifiedDetails *)request->historyReadDetails.content.decoded.data;
    ...
    UA_HistoryData **historyData =
        (UA_HistoryData **)UA_calloc(request->nodesToReadSize, sizeof(UA_HistoryData *));
    if(!historyData) {
    ...
    for(size_t i = 0; i < response->resultsSize; ++i) {
        ...
        response->results[i].historyData.content.decoded.type =
    ...
    server->config.historyDatabase.readRaw(
        server, server->config.historyDatabase.context, &session->sessionId,
        session->sessionHandle, &request->requestHeader, details,
           ...
           &UA_TYPES[UA_TYPES_UPDATEDATADETAILS]) {
            if(server->config.historyDatabase.updateData) {
                server->config.historyDatabase.updateData(
                    ...
                    (UA_UpdateDataDetails *)request->historyUpdateDetails[i]
            ...
            continue;
        } else if(request->historyUpdateDetails[i].content.decoded.type ==
            ...
            if(server->config.historyDatabase.deleteRawModified) {
                server->config.historyDatabase.deleteRawModified(
                    server, server->config.historyDatabase.context, &session->sessionId,
                    session->sessionHandle, &request->requestHeader,
                    (UA_DeleteRawModifiedDetails *)request->historyUpdateDetails[i]
                        .content.decoded.data,
                    &response->results[i]);
            } else {
                response->results[i].statusCode = UA_STATUSCODE_BADNOTSUPPORTED;
