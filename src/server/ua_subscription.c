...
UA_Subscription_new(UA_Session *session, UA_UInt32 subscriptionId) {
    ...
    newSub->state =
        UA_SUBSCRIPTIONSTATE_NORMAL; /* The first publish response is sent immediately */
...
removeOldestRetransmissionMessage(UA_Session *session) {
    ...
    LIST_FOREACH(sub, &session->serverSubscriptions, listEntry) {
        ...
        if(!first)
            continue;
        if(!oldestEntry ||
           oldestEntry->message.publishTime > first->message.publishTime) {
            oldestEntry = first;
            oldestSub = sub;
        }
    }
...
prepareNotificationMessage(UA_Server *server, UA_Subscription *sub,
    ...
    message->notificationData =
        (UA_ExtensionObject *)UA_Array_new(2, &UA_TYPES[UA_TYPES_EXTENSIONOBJECT]);
    ...
    if(sub->dataChangeNotifications > 0) {
        ...
        message->notificationData->content.decoded.type =
        ...
        dcn->monitoredItems = (UA_MonitoredItemNotification *)UA_Array_new(
            dcnSize, &UA_TYPES[UA_TYPES_MONITOREDITEMNOTIFICATION]);
    ...
    if(sub->statusChangeNotifications > 0) {
        ...
        message->notificationData[notificationDataIdx].encoding =
        ...
        enl->events = (UA_EventFieldList *)UA_Array_new(
            enlSize, &UA_TYPES[UA_TYPES_EVENTFIELDLIST]);
        if(!enl->events) {
            UA_NotificationMessage_deleteMembers(message);
            return UA_STATUSCODE_BADOUTOFMEMORY;
        }
    ...
    size_t totalNotifications =
        0;             /* How many notifications were moved to the response overall? */
    ...
    TAILQ_FOREACH_SAFE(notification, &sub->notificationQueue, globalEntry,
                       notification_tmp) {
        ...
        if(mon->monitoredItemType == UA_MONITOREDITEMTYPE_CHANGENOTIFY) {
            ...
            UA_DataValue_init(
                &notification->data.value); /* Reset after the value has been moved */
            dcnPos++;
        }
#ifdef UA_ENABLE_SUBSCRIPTIONS_EVENTS
...
UA_Subscription_publish(UA_Server *server, UA_Subscription *sub) {
    ...
    if(!channel || !pre) {
        UA_LOG_DEBUG_SESSION(
            &server->config.logger, sub->session,
    ...
    if(notifications > 0) {
        if(server->config.enableRetransmissionQueue) {
            ...
            retransmission = (UA_NotificationMessageEntry *)UA_malloc(
                sizeof(UA_NotificationMessageEntry));
            if(!retransmission) {
...
Subscription_registerPublishCallback(UA_Server *server, UA_Subscription *sub) {
    ...
    UA_StatusCode retval = UA_Server_addRepeatedCallback(
        server, (UA_ServerCallback)publishCallback, sub,
        (UA_UInt32)sub->publishingInterval, &sub->publishCallbackId);
    if(retval != UA_STATUSCODE_GOOD)
        return retval;
