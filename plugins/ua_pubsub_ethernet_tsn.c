...
} UA_PubSubChannelDataEthernetTSN;
...
UA_PubSubChannelEthernetTSN_open(const UA_PubSubConnectionConfig *connectionConfig) {

    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_SERVER, "Open PubSub ethernet connection.");

    /* allocate and init memory for the ethernet specific internal data */
    UA_PubSubChannelDataEthernetTSN *channelDataEthernet =
        (UA_PubSubChannelDataEthernetTSN *)UA_calloc(1, sizeof(*channelDataEthernet));
    ...
    sockTxtime.flags = (connectionConfig->soTxTimeEnbableDeadlineMode |
    ...
    if(UA_bind(sockFd, (struct sockaddr *)&sll, sizeof(sll)) < 0) {
     ...
     if(UA_setsockopt(sockFd, SOL_SOCKET, SO_PRIORITY, &connectionConfig->soTxTimePriority, sizeof(UA_UInt32)) < 0)
         UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_SERVER,
 ...
 */
static UA_StatusCode
UA_PubSubChannelEthernetTSN_regist(UA_PubSubChannel *channel,
                                UA_ExtensionObject *transportSettings,
                                void (*notUsedHere)(UA_ByteString *encodedBuffer,
                                                    UA_ByteString *topic)) {
    UA_PubSubChannelDataEthernetTSN *channelDataEthernet =
        (UA_PubSubChannelDataEthernetTSN *)channel->handle;
 ...
 */
static UA_StatusCode
UA_PubSubChannelEthernetTSN_unregist(UA_PubSubChannel *channel,
                                  UA_ExtensionObject *transportSettings) {
    UA_PubSubChannelDataEthernetTSN *channelDataEthernet =
        (UA_PubSubChannelDataEthernetTSN *)channel->handle;
 ...
 */
static UA_StatusCode
UA_PubSubChannelEthernetTSN_send(UA_PubSubChannel *channel,
                              UA_ExtensionObject *transportSettings,
    ...
    sinll.sll_ifindex = channelDataEthernet->ifindex;
    sinll.sll_protocol = htons(ETHERTYPE_UADP);
 ...
 */
static UA_StatusCode
UA_PubSubChannelEthernetTSN_receive(UA_PubSubChannel *channel, UA_ByteString *message,
                                 UA_ExtensionObject *transportSettings,
                                 UA_UInt32 timeout) {
    UA_PubSubChannelDataEthernetTSN *channelDataEthernet =
        (UA_PubSubChannelDataEthernetTSN *)channel->handle;
 ...
 */
static UA_StatusCode
UA_PubSubChannelEthernetTSN_close(UA_PubSubChannel *channel) {
 ...
 */
static UA_PubSubChannel *
TransportLayerEthernetTSN_addChannel(UA_PubSubConnectionConfig *connectionConfig) {
    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "PubSub channel requested");
    UA_PubSubChannel *pubSubChannel = UA_PubSubChannelEthernetTSN_open(connectionConfig);
    if(pubSubChannel) {
        pubSubChannel->regist = UA_PubSubChannelEthernetTSN_regist;
        pubSubChannel->unregist = UA_PubSubChannelEthernetTSN_unregist;
        pubSubChannel->send = UA_PubSubChannelEthernetTSN_send;
        pubSubChannel->receive = UA_PubSubChannelEthernetTSN_receive;
        pubSubChannel->close = UA_PubSubChannelEthernetTSN_close;
...
UA_PubSubTransportLayerEthernetTSN() {
    UA_PubSubTransportLayer pubSubTransportLayer;
    pubSubTransportLayer.transportProfileUri =
        UA_STRING("http://opcfoundation.org/UA-Profile/Transport/pubsub-eth-uadp");
    pubSubTransportLayer.createPubSubChannel = &TransportLayerEthernetTSN_addChannel;
