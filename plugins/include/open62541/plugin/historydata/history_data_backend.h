...
struct UA_HistoryDataBackend {
     ...
     * sessionId and sessionContext identify the session that wants to read historical
     * data. nodeId is the node for which the value shall be stored. value is the value
     * which shall be stored. historizing is the historizing flag of the node identified
     * by nodeId. If sessionId is NULL, the historizing flag is invalid and must not be
     * used. */
    UA_StatusCode (*serverSetHistoryData)(UA_Server *server, void *hdbContext,
     ...
     * hdbContext is the context of the UA_HistoryDataBackend.
     * sessionId and sessionContext identify the session that wants to read historical
     * data. backend is the HistoryDataBackend whose storage is to be queried. start is
     * the start time of the HistoryRead request. end is the end time of the HistoryRead
     * request. nodeId is the node id of the node for which historical data is requested.
     * maxSizePerResponse is the maximum number of items per response the server can
     * provide. numValuesPerNode is the maximum number of items per response the client
     * wants to receive. returnBounds determines if the client wants to receive bounding
     * values. timestampsToReturn contains the time stamps the client is interested in.
     ...
     * continuationPoint is the continuation point the client wants to release or start
     * from. outContinuationPoint is the continuation point that gets passed to the client
     * by the HistoryRead service.
     * result contains the result histoy data that gets passed to the client. */
    UA_StatusCode (*getHistoryData)(
        UA_Server *server, const UA_NodeId *sessionId, void *sessionContext,
        const UA_HistoryDataBackend *backend, const UA_DateTime start,
        const UA_DateTime end, const UA_NodeId *nodeId, size_t maxSizePerResponse,
        UA_UInt32 numValuesPerNode, UA_Boolean returnBounds,
        UA_TimestampsToReturn timestampsToReturn, UA_NumericRange range,
        UA_Boolean releaseContinuationPoints, const UA_ByteString *continuationPoint,
     ...
     * hdbContext is the context of the UA_HistoryDataBackend.
     * sessionId and sessionContext identify the session that wants to read historical
     * data. nodeId is the node id of the node for which the matching value shall be
     * found. timestamp is the timestamp of the requested index.
     * strategy is the matching strategy which shall be applied in finding the index. */
    size_t (*getDateTimeMatch)(UA_Server *server, void *hdbContext,
     ...
     * hdbContext is the context of the UA_HistoryDataBackend.
     * sessionId and sessionContext identify the session that wants to read historical
     * data. nodeId is the node id of the node for which the end of storage shall be
     * returned. */
    size_t (*getEnd)(UA_Server *server, void *hdbContext, const UA_NodeId *sessionId,
                     void *sessionContext, const UA_NodeId *nodeId);
     ...
     * hdbContext is the context of the UA_HistoryDataBackend.
     * sessionId and sessionContext identify the session that wants to read historical
     * data. nodeId is the node id of the node for which the index of the last element
     *        shall be returned. */
    size_t (*lastIndex)(UA_Server *server, void *hdbContext, const UA_NodeId *sessionId,
                        void *sessionContext, const UA_NodeId *nodeId);
     ...
     * hdbContext is the context of the UA_HistoryDataBackend.
     * sessionId and sessionContext identify the session that wants to read historical
     * data. nodeId is the node id of the node for which the index of the first element
     * shall be returned. */
    size_t (*firstIndex)(UA_Server *server, void *hdbContext, const UA_NodeId *sessionId,
                         void *sessionContext, const UA_NodeId *nodeId);
     ...
     * hdbContext is the context of the UA_HistoryDataBackend.
     * sessionId and sessionContext identify the session that wants to read historical
     * data. nodeId is the node id of the node for which the number of elements shall be
     * returned. startIndex is the index of the first element in the range. endIndex is
     * the index of the last element in the range. */
    size_t (*resultSize)(UA_Server *server, void *hdbContext, const UA_NodeId *sessionId,
     ...
     * hdbContext is the context of the UA_HistoryDataBackend.
     * sessionId and sessionContext identify the session that wants to read historical
     * data. nodeId is the node id of the node for which the data values shall be copied.
     ...
     * releaseContinuationPoints determines if the continuation points shall be released.
     * continuationPoint is a continuation point the client wants to release or start
     * from. outContinuationPoint is a continuation point which will be passed to the
     * client. providedValues contains the number of values that were copied. values
     * contains the values that have been copied from the database. */
     ...
     * hdbContext is the context of the UA_HistoryDataBackend.
     * sessionId and sessionContext identify the session that wants to read historical
     * data. nodeId is the node id of the node for which the data value shall be returned.
    ...
    UA_Boolean (*boundSupported)(UA_Server *server, void *hdbContext,
                                 const UA_NodeId *sessionId, void *sessionContext,
     ...
     * hdbContext is the context of the UA_HistoryDataBackend.
     * sessionId and sessionContext identify the session that wants to read historical
     * data. nodeId is the node id of the node for which the capability to return
     *        certain timestamps shall be queried. */
    UA_Boolean (*timestampsToReturnSupported)(
