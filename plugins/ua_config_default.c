...
addDefaultNetworkLayers(UA_ServerConfig *conf, UA_UInt16 portNumber,
    ...
    /* Add a network layer */
    conf->networkLayers =
        (UA_ServerNetworkLayer *)UA_malloc(sizeof(UA_ServerNetworkLayer));
    ...
    conf->networkLayers[0] = UA_ServerNetworkLayerTCP(config, portNumber, &conf->logger);
...
UA_ServerConfig_new_customBuffer(UA_UInt16 portNumber, const UA_ByteString *certificate,
    ...
    if(addDefaultNetworkLayers(conf, portNumber, sendBufferSize, recvBufferSize) !=
       UA_STATUSCODE_GOOD) {
        UA_ServerConfig_delete(conf);
        return NULL;
    }
    ...
    retval = UA_SecurityPolicy_None(&conf->securityPolicies[0], NULL, localCertificate,
                                    &conf->logger);
    if(retval != UA_STATUSCODE_GOOD) {
        UA_ServerConfig_delete(conf);
        return NULL;
...
UA_ServerConfig *
UA_ServerConfig_new_basic128rsa15(UA_UInt16 portNumber, const UA_ByteString *certificate,
    ...
    UA_StatusCode retval = UA_CertificateVerification_Trustlist(
        &conf->certificateVerification, trustList, trustListSize, revocationList,
        revocationListSize);
    if(retval != UA_STATUSCODE_GOOD) {
        UA_ServerConfig_delete(conf);
        return NULL;
    ...
    conf->securityPolicies =
        (UA_SecurityPolicy *)UA_malloc(sizeof(UA_SecurityPolicy) * 2);
    if(!conf->securityPolicies) {
        UA_ServerConfig_delete(conf);
        return NULL;
    }
    ...
    retval = UA_SecurityPolicy_Basic128Rsa15(
        &conf->securityPolicies[1], &conf->certificateVerification, localCertificate,
    ...
    conf->endpoints =
        (UA_EndpointDescription *)UA_malloc(sizeof(UA_EndpointDescription) * 3);
    if(!conf->endpoints) {
        UA_ServerConfig_delete(conf);
        return NULL;
    }
...
UA_ServerConfig *
UA_ServerConfig_new_basic256sha256(UA_UInt16 portNumber, const UA_ByteString *certificate,
                                   const UA_ByteString *privateKey,
                                   const UA_ByteString *trustList, size_t trustListSize,
                                   const UA_ByteString *revocationList,
                                   size_t revocationListSize) {
...
UA_ServerConfig *
UA_ServerConfig_new_allSecurityPolicies(
    UA_UInt16 portNumber, const UA_ByteString *certificate,
    ...
    conf->securityPoliciesSize = 0;
    conf->securityPolicies =
        (UA_SecurityPolicy *)UA_malloc(sizeof(UA_SecurityPolicy) * 4);
    if(!conf->securityPolicies) {
        UA_ServerConfig_delete(conf);
        return NULL;
    }
    ...
    ++conf->securityPoliciesSize;
    retval = UA_SecurityPolicy_Basic256Sha256(
        &conf->securityPolicies[3], &conf->certificateVerification, localCertificate,
        localPrivateKey, &conf->logger);
    if(retval != UA_STATUSCODE_GOOD) {
        UA_ServerConfig_delete(conf);
        return NULL;
    ...
    conf->endpointsSize = 0;
    conf->endpoints =
        (UA_EndpointDescription *)UA_malloc(sizeof(UA_EndpointDescription) * 7);
    if(!conf->endpoints) {
        UA_ServerConfig_delete(conf);
        return NULL;
    }
    ...
    retval =
        createEndpoint(conf, &conf->endpoints[conf->endpointsSize],
...
UA_ServerConfig_addPubSubTransportLayer(UA_ServerConfig *config,
    ...
    if(config->pubsubTransportLayersSize == 0) {
        ...
        config->pubsubTransportLayers = (UA_PubSubTransportLayer *)UA_realloc(
            config->pubsubTransportLayers,
