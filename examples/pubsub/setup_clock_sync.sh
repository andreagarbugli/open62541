...
#
setup_ptp_master() {
	taskset -c 1 chrt 98 ptp4l -2 -i $INTERFACE $PTP4L_VERBOSE &
	# ptp4l -f /etc/linuxptp/ptp4l.conf -i $INTERFACE $PTP4L_VERBOSE &
}

setup_ptp_master_and_sync() {
	taskset -c 1 chrt 98 phc2sys -s $INTERFACE -c CLOCK_REALTIME -w $PHC2SYS_VERBOSE &
	setup_ptp_master
}
...
setup_ptp_slave() {
	phc2sys -s $INTERFACE -c CLOCK_REALTIME -w $PHC2SYS_VERBOSE &
	ptp4l -2 -s -i $INTERFACE $PTP4L_VERBOSE &
}

setup_gptp() {
	taskset -c 0 chrt 80 ptp4l -f ./gPTP.cfg -i $INTERFACE $PTP4L_VERBOSE &
	taskset -c 0 chrt 79 phc2sys -f ./phc2sys.cfg -s $INTERFACE -c CLOCK_REALTIME -w  $PHC2SYS_VERBOSE &
}
...
while getopts "Mmgasvi:" opt; do
	...
	g)	ptp_master_mode=g ;;
	a)	ptp_master_mode=a ;;
	v)	PTP4L_VERBOSE='-m --summary_interval=1' ;
		PHC2SYS_VERBOSE='-m -u 1' ;;
	*)	exit -1 ;;
	esac
done
...
if [ ${ptp_master_mode} = y ]; then
	...
	setup_ptp_slave
	adjust_clock_tai_offset
elif [ ${ptp_master_mode} = g ]; then
	TAI_OFFSET=0
	setup_gptp
	adjust_clock_tai_offset
elif [ ${ptp_master_mode} = a ]; then
	adjust_clock_tai_offset
else 
