...
UA_DateTime_now(void) {
    UA_DateTime microSeconds =
        ((UA_DateTime)xTaskGetTickCount()) * (1000000 / configTICK_RATE_HZ);
...
UA_DateTime_localTimeUtcOffset(void) {
    return 0;  // TODO: this is set to zero since UA_DateTime_now() is just the local
               // time, and not UTC.
}
...
UA_DateTime_nowMonotonic(void) {
    return (((UA_DateTime)xTaskGetTickCount()) * 1000 / configTICK_RATE_HZ) *
           UA_DATETIME_MSEC;
}
