...
static char *errorStrings[] = {"Error 0",
                               "",
...
#if defined(__MINGW32__)  // mingw defines SOCKET as long long unsigned int, giving errors
                          // in logging and when comparing with UA_Int32
...
#define UA_recvfrom(sockfd, buf, len, flags, src_addr, addrlen)                          \
    recvfrom(sockfd, (char *)(buf), (int)(len), flags, src_addr, addrlen)
...
#define UA_snprintf(source, size, string, ...)                                           \
    _snprintf_s(source, size, _TRUNCATE, string, __VA_ARGS__)
