 ...
 * attribute. */
UA_StatusCode UA_EXPORT
__UA_Server_read(UA_Server *server, const UA_NodeId *nodeId, UA_AttributeId attributeId,
                 void *v);
...
UA_Server_writeHistorizing(UA_Server *server, const UA_NodeId nodeId,
    ...
    return __UA_Server_write(server, &nodeId, UA_ATTRIBUTEID_HISTORIZING,
...
typedef void (*UA_Server_registerServerCallback)(
...
typedef void (*UA_Server_serverOnNetworkCallback)(
    const UA_ServerOnNetwork *serverOnNetwork, UA_Boolean isServerAnnounce,
...
typedef void (*UA_Server_EventNotificationCallback)(UA_Server *server, UA_UInt32 monId,
...
UA_Server_addVariableNode(UA_Server *server, const UA_NodeId requestedNewNodeId,
                          ...
                          UA_NodeId *outNewNodeId) {
    return __UA_Server_addNode(
        server, UA_NODECLASS_VARIABLE, &requestedNewNodeId, &parentNodeId,
        &referenceTypeId, browseName, &typeDefinition, (const UA_NodeAttributes *)&attr,
        &UA_TYPES[UA_TYPES_VARIABLEATTRIBUTES], nodeContext, outNewNodeId);
}
...
UA_Server_addVariableTypeNode(UA_Server *server, const UA_NodeId requestedNewNodeId,
                              ...
                              UA_NodeId *outNewNodeId) {
    return __UA_Server_addNode(
        server, UA_NODECLASS_VARIABLETYPE, &requestedNewNodeId, &parentNodeId,
        &referenceTypeId, browseName, &typeDefinition, (const UA_NodeAttributes *)&attr,
        &UA_TYPES[UA_TYPES_VARIABLETYPEATTRIBUTES], nodeContext, outNewNodeId);
}
...
UA_Server_addObjectNode(UA_Server *server, const UA_NodeId requestedNewNodeId,
                        ...
                        UA_NodeId *outNewNodeId) {
    return __UA_Server_addNode(
        server, UA_NODECLASS_OBJECT, &requestedNewNodeId, &parentNodeId, &referenceTypeId,
        browseName, &typeDefinition, (const UA_NodeAttributes *)&attr,
        &UA_TYPES[UA_TYPES_OBJECTATTRIBUTES], nodeContext, outNewNodeId);
}
...
UA_Server_addObjectTypeNode(UA_Server *server, const UA_NodeId requestedNewNodeId,
                            const UA_NodeId parentNodeId, const UA_NodeId referenceTypeId,
                            const UA_QualifiedName browseName,
                            const UA_ObjectTypeAttributes attr, void *nodeContext,
                            UA_NodeId *outNewNodeId) {
    return __UA_Server_addNode(
        server, UA_NODECLASS_OBJECTTYPE, &requestedNewNodeId, &parentNodeId,
        &referenceTypeId, browseName, &UA_NODEID_NULL, (const UA_NodeAttributes *)&attr,
        &UA_TYPES[UA_TYPES_OBJECTTYPEATTRIBUTES], nodeContext, outNewNodeId);
}
...
UA_Server_addViewNode(UA_Server *server, const UA_NodeId requestedNewNodeId,
    ...
    return __UA_Server_addNode(
        server, UA_NODECLASS_VIEW, &requestedNewNodeId, &parentNodeId, &referenceTypeId,
        browseName, &UA_NODEID_NULL, (const UA_NodeAttributes *)&attr,
        &UA_TYPES[UA_TYPES_VIEWATTRIBUTES], nodeContext, outNewNodeId);
}
...
UA_Server_addReferenceTypeNode(UA_Server *server, const UA_NodeId requestedNewNodeId,
                               ...
                               UA_NodeId *outNewNodeId) {
    return __UA_Server_addNode(
        server, UA_NODECLASS_REFERENCETYPE, &requestedNewNodeId, &parentNodeId,
        &referenceTypeId, browseName, &UA_NODEID_NULL, (const UA_NodeAttributes *)&attr,
        &UA_TYPES[UA_TYPES_REFERENCETYPEATTRIBUTES], nodeContext, outNewNodeId);
}
...
UA_Server_addDataTypeNode(UA_Server *server, const UA_NodeId requestedNewNodeId,
                          const UA_NodeId parentNodeId, const UA_NodeId referenceTypeId,
                          const UA_QualifiedName browseName,
                          const UA_DataTypeAttributes attr, void *nodeContext,
                          UA_NodeId *outNewNodeId) {
    return __UA_Server_addNode(
        server, UA_NODECLASS_DATATYPE, &requestedNewNodeId, &parentNodeId,
        &referenceTypeId, browseName, &UA_NODEID_NULL, (const UA_NodeAttributes *)&attr,
        &UA_TYPES[UA_TYPES_DATATYPEATTRIBUTES], nodeContext, outNewNodeId);
}
...
UA_Server_addMethodNode(UA_Server *server, const UA_NodeId requestedNewNodeId,
                        ...
                        const UA_Argument *inputArguments, size_t outputArgumentsSize,
                        const UA_Argument *outputArguments, void *nodeContext,
                        UA_NodeId *outNewNodeId) {
    return UA_Server_addMethodNodeEx(server, requestedNewNodeId, parentNodeId,
                                     ...
                                     NULL, outputArgumentsSize, outputArguments,
                                     UA_NODEID_NULL, NULL, nodeContext, outNewNodeId);
}
 ...
 * Events
 * ------
 * The method ``UA_Server_createEvent`` creates an event and represents it as node. The
 * node receives a unique `EventId` which is automatically added to the node. The method
 * returns a `NodeId` to the object node which represents the event through ``outNodeId``.
 * The `NodeId` can be used to set the attributes of the event. The generated `NodeId` is
 * always numeric. ``outNodeId`` cannot be
 * ``NULL``.
 *
 * Note: In order to see an event in UAExpert, the field `Time` must be given a value!
 *
 * The method ``UA_Server_triggerEvent`` "triggers" an event by adding it to all monitored
 * items of the specified origin node and those of all its parents. Any filters specified
 * by the monitored items are automatically applied. Using this method deletes the node
 * generated by ``UA_Server_createEvent``. The `EventId` for the new event is generated
 * automatically and is returned through ``outEventId``. ``NULL`` can be passed if the
 * `EventId` is not needed. ``deleteEventNode`` specifies whether the node representation
 * of the event should be deleted after invoking the method. This can be useful if events
 * with the similar attributes are triggered frequently. ``UA_TRUE`` would
 ...
 * @param server The server object
 * @param eventNodeId The NodeId of the node representation of the event which should be
 triggered
 * @param outEvent the EventId of the new event
 * @param deleteEventNode Specifies whether the node representation of the event should be
 deleted
